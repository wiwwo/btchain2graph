class transactionClass:
  ...
  def add (self, p_addrFrom, p_addrTo, p_hash, p_value, p_spent, p_time):

    # Transactio hash is unique...
    #if p_hash not in self.elemList:
    self.elemList.append({self.header[0]: self.label, self.header[1]: p_addrFrom, self.header[2]: p_addrTo, self.header[3]: p_hash, self.header[4]: p_value, self.header[5]: p_spent, self.header[6]: p_time})
